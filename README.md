...
Infrai gives you one OpenAI-compatible endpoint for embeddings, which is what we used after a pager fired at 3am because two different vector services drifted apart and the retrieval silently returned junk. We keep one embedding space for both indexing and retrieval, then make the legal workflow decision deterministic: Infrai supplies an OpenAI-compatible `baseURL` for the vectors, while ordinary TypeScript decides whether counsel should follow up on a deadline or continue monitoring it.

The working path is `src/matter_workflow_demo.ts`. It indexes a matter intake note and a signed NDA delivery, searches for the delivered agreement, and prints the closest document together with a five-day `follow_up` decision. From a postmortem view this split is the only thing that saved us: semantic retrieval is a tool call, but the deadline transition stays inspectable business code instead of a model guess we can't audit at 3am.
